...
ep2 = "9eb93f5a-6c2a-11ee-ad2b-9197a4554f81"    # local, HDD1
# root_path = f"{ep1}:Halos/FoF"                # local for SD
root_path = f"{ep1}:3D_cubes"

folders2download = ['fiducial', 'zeldovich', 'h_m', 'h_p', 'Mnu_p', 'Mnu_pp' ,'Mnu_ppp', \
...
#     print(ls(root_path))
#     print(root_dir_raw)
#     if root_dir_raw in folders2download:

#         root_dir = root_dir_raw.strip()

#         inner_path = f"{root_path}/{root_dir}"
#         number2download = 0
#         if "fiducial" in root_dir_raw:
#             number2download = 2000
#         elif "zeldovich" in root_dir_raw:
#             number2download = 500
#         else:
#             number2download = 350
#         for inner_dir in tqdm(ls(inner_path)):
#             try:
#                 if int(inner_dir[:-1]) >= number2download:
#                     continue
#             except Exception:
#                 pass
